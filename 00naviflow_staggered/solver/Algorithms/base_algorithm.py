...
class BaseAlgorithm(ABC):
    ...
    def apply_boundary_conditions(self):
        ...
        # --- COMMENTED OUT DIRECT APPLICATION ---
        # u_bc_out, v_bc_out = self.bc_manager.apply_velocity_boundary_conditions(
        #     u_in, v_in, nx, ny
        # )
        # Instead, assume solvers handle BCs internally based on mesh info
        u_bc_out = u_in # Keep original values
        v_bc_out = v_in # Keep original values
        # --- END COMMENT ---
    ...
    def set_boundary_condition(self, boundary, condition_type, values=None):
        ...
        # --- ADDED: Update mesh's internal BC dictionary --- 
        if hasattr(self.mesh, '_boundary_conditions'):
            self.mesh._boundary_conditions = self.bc_manager.to_dict()
        # --- END ADDED ---

        # Apply the boundary condition (now potentially just ensures manager state)
        self.apply_boundary_conditions()
