...
class SimpleSolver(BaseAlgorithm):
    ...
    def reimpose_velocity_bc_lid_driven(self):
        """
        Reimpose velocity boundary conditions hardcoded for lid-driven cavity.
        Assumes fields are stored as 1D arrays and StructuredMesh layout.
        """
        nx, ny = self.mesh.n_cells_x, self.mesh.n_cells_y

        # Reshape u, v to 2D for easier indexing
        u_2d = self.u.reshape((nx, ny))
        v_2d = self.v.reshape((nx, ny))

        # Top boundary (moving lid)
        u_2d[:, -1] = 1.0  # Top wall u = 1
        v_2d[:, -1] = 0.0  # Top wall v = 0
    ...
    def reimpose_pressure_bc_lid_driven(self):
        """
        Reimpose pressure boundary conditions hardcoded for lid-driven cavity.
        Enforces zero-gradient (Neumann) on boundaries, and pins one point to 0.
        """
        nx, ny = self.mesh.n_cells_x, self.mesh.n_cells_y

        # Reshape p to 2D for easier indexing
        p_2d = self.p.reshape((nx, ny))

        # Left and right (Neumann zero-gradient)
        p_2d[0, :] = p_2d[1, :]
        p_2d[-1, :] = p_2d[-2, :]

        # Bottom and top (Neumann zero-gradient)
        p_2d[:, 0] = p_2d[:, 1]
        p_2d[:, -1] = p_2d[:, -2]

        # Pin pressure at (0,0) to 0
        p_2d[0, 0] = 0.0
            ...
            while iteration <= max_iterations: #and total_res_check > tolerance:
                ...
                if hasattr(self.pressure_solver, 'solve_pressure_correction'):
                    p_prime, p_res_info = self.pressure_solver.solve_pressure_correction(
                        ...
                        relaxation_factor=self.alpha_p
                    )
                else:
                ...
                # Reimpose hardcoded pressure BCs
                # self.reimpose_pressure_bc_lid_driven() # REMOVED
                ...
                # Reimpose hardcoded velocity BCs
                # self.reimpose_velocity_bc_lid_driven() # REMOVED
                ...
                try:
                    # Calculate d_avg as average of d_u and d_v
                    d_avg = 0.5 * (self._tmp_d_u + self._tmp_d_v)
                    rhs = get_rhs(self.mesh, self.fluid.get_density(), self.u, self.v, p=self.p, d_avg=d_avg) 
