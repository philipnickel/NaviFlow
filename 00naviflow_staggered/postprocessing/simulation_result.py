...
class SimulationResult:
    ...
    def plot_combined_results(self, title=None, filename=None, show=True):
        ...
        dx = (self.mesh.xmax - self.mesh.xmin) / self.mesh.n_cells_x
        dy = (self.mesh.ymax - self.mesh.ymin) / self.mesh.n_cells_y
        ...
        # Reshape velocity fields if they are 1D before passing to plotting
        u_reshaped = self.u
        v_reshaped = self.v
        p_reshaped = self.p # Also reshape pressure if needed by plotting function
        if self.u.ndim == 1:
            # Assuming C order (row-major) now
            u_reshaped = self.u.reshape((nx, ny), order='F') 
        if self.v.ndim == 1:
            v_reshaped = self.v.reshape((nx, ny), order='F')
        if self.p.ndim == 1:
             p_reshaped = self.p.reshape((nx, ny), order='F')
             
        plot_combined_results_matrix(
            u_reshaped, v_reshaped, p_reshaped, x, y, 
    ...
    def get_max_divergence(self):
        ...
        if self.divergence is None:
            ...
            nx, ny = self.mesh.get_dimensions() # Get dimensions first
            dx = (self.mesh.xmax - self.mesh.xmin) / self.mesh.n_cells_x
            dy = (self.mesh.ymax - self.mesh.ymin) / self.mesh.n_cells_y
            
            # Reshape velocity fields if they are 1D
            u_reshaped = self.u
            v_reshaped = self.v
            if self.u.ndim == 1:
                # Assuming Fortran order based on typical CFD indexing (column-major)
                u_reshaped = self.u.reshape((nx, ny), order='F') 
            if self.v.ndim == 1:
                v_reshaped = self.v.reshape((nx, ny), order='F')

            self.divergence = calculate_divergence(u_reshaped, v_reshaped, dx, dy)

        # Get dimensions again for mask creation (may not be needed if divergence is now 2D)
        if self.divergence.ndim == 2:
            nx_div, ny_div = self.divergence.shape
        else:
             # If divergence calculation somehow returned 1D, handle appropriately
             # This part might need adjustment based on how calculate_divergence returns errors or handles 1D input
             # For now, assume calculate_divergence returns 2D or raises an error
             nx_div, ny_div = self.mesh.get_dimensions() # Fallback, might be incorrect if divergence is 1D
        ...
        if self.divergence.ndim == 2:
             ...
             # Handle 1D mask if needed - this logic might be complex depending on 1D representation
             # For now, assume 2D
             pass 
