...
def plot_final_residuals(u_residual_field, v_residual_field, p_residual_field, mesh, title=None, filename=None, show=True, output_dir=None,
    ...
    # --- Reshape Residual Fields if 1D --- 
    nx, ny = mesh.get_dimensions()
    u_res_field_2d = u_residual_field
    v_res_field_2d = v_residual_field
    p_res_field_2d = p_residual_field
    if u_residual_field.ndim == 1:
        # For now, assuming cell-centered shape, adjust if needed
        try:
            # Assuming C order (row-major) now
            u_res_field_2d = u_residual_field.reshape((nx, ny), order='C')
        except ValueError:
            print(f"Warning: Could not reshape u_residual_field ({u_residual_field.size}) to ({nx}, {ny}). Using 1D.")
    if v_residual_field.ndim == 1:
        try:
            v_res_field_2d = v_residual_field.reshape((nx, ny), order='C')
        except ValueError:
             print(f"Warning: Could not reshape v_residual_field ({v_residual_field.size}) to ({nx}, {ny}). Using 1D.")
    if p_residual_field.ndim == 1:
        try:
            p_res_field_2d = p_residual_field.reshape((nx, ny), order='C')
        except ValueError:
             print(f"Warning: Could not reshape p_residual_field ({p_residual_field.size}) to ({nx}, {ny}). Using 1D.")
    # --- End Reshape ---
             
    # Use reshaped fields from here on
    final_p_residual = np.abs(p_res_field_2d)
    final_u_residual = np.abs(u_res_field_2d)
    final_v_residual = np.abs(v_res_field_2d)
    
    # nx, ny = mesh.get_dimensions() # Already have nx, ny
    dx = (mesh.xmax - mesh.xmin) / mesh.n_cells_x
    dy = (mesh.ymax - mesh.ymin) / mesh.n_cells_y

    # --- Slice data and coordinates to get interior points only ---
    # Pressure residuals (cell centers: 1 to nx-2, 1 to ny-2)
    # Check dimensions before slicing to avoid errors if reshape failed
    p_res_interior = np.array([]) # Default to empty
    if final_p_residual.ndim == 2:
        p_res_interior = final_p_residual[1:nx-1, 1:ny-1]
    x_p_interior = np.linspace(dx/2 + dx, mesh.xmax - dx/2 - dx, nx-2) # Interior cell centers x
    y_p_interior = np.linspace(dy/2 + dy, mesh.ymax - dy/2 - dy, ny-2) # Interior cell centers y

    # U-momentum residuals (u-faces: i=1 to nx-1, cell centers j=1 to ny-2)
    # Note: u_residual_field might have shape (nx+1, ny) or (nx, ny) depending on convention.
    # Adjust slicing based on actual reshaped dimension.
    u_res_interior = np.array([]) # Default
    if final_u_residual.ndim == 2:
        u_res_interior = final_u_residual[1:nx, 1:ny-1] 
        # Assuming shape (nx,ny) for now. If it's (nx+1, ny), adjust slice: final_u_residual[1:nx, 1:ny-1]
    # Coordinates for u-faces (x) and corresponding cell centers (y)
    x_u_interior = np.linspace(dx, mesh.xmax - dx, nx-1) # Interior u-face x-locations 
    y_u_interior = y_p_interior # Use the same interior y cell centers

    # V-momentum residuals (cell centers i=1 to nx-2, v-faces j=1 to ny-1)
    # Note: v_residual_field shape might be (nx, ny+1) or (nx, ny)
    v_res_interior = np.array([]) # Default
    if final_v_residual.ndim == 2:
        v_res_interior = final_v_residual[1:nx-1, 1:ny]
        # Assuming shape (nx,ny). If it's (nx, ny+1), adjust slice: final_v_residual[1:nx-1, 1:ny]
    # Coordinates for cell centers (x) and corresponding v-faces (y)
    x_v_interior = x_p_interior # Use the same interior x cell centers
    y_v_interior = np.linspace(dy, mesh.ymax - dy, ny-1) # Interior v-face y-locations
    # --- End slicing ---
    ...
    def plot_residual_field_native(ax, data, x_coords, y_coords, field_title, log_scale=False):
        ...
        if valid_plot:            
            ...
            if X.shape == data.shape:
                ...
                ax.set_xlim(dx, mesh.xmax - dx)
                ax.set_ylim(dy, mesh.ymax - dy)
            else:
