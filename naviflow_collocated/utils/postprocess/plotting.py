...
def plot_streamlines(x, y, U, output_path, experiment=None, Re=None, sim_id=None, return_as_array=False, time_val=None, hide_colorbar=False):
    ...
    # Create grid for streamline plotting
    n_grid = 100
    xi = np.linspace(np.min(x), np.max(x), n_grid)
    yi = np.linspace(np.min(y), np.max(y), n_grid)
    Xg, Yg = np.meshgrid(xi, yi)
    ...
    Ug = griddata((x, y), U[:, 0], (Xg, Yg), method='cubic')
    Vg = griddata((x, y), U[:, 1], (Xg, Yg), method='cubic')
    ...
    # Add title with experiment name and Reynolds number
    title_str = "Streamlines"
    if experiment is not None:
        title_str += f" | {experiment}"
    if Re is not None:
        title_str += f" | Re={Re}"
    ...
    # Add time value if provided
    if time_val is not None:
        ax.text(0.02, 0.98, f"Time = {time_val:.4f}s", 
...
def save_individual_field_plots(x, y, U, velocity_magnitude, p, experiment, Re, sim_id, results_dir):
    ...
    # streamlines only
    plot_streamlines(x, y, U, os.path.join(plots_dir, "streamlines_legacy.pdf"), experiment=experiment, Re=Re, sim_id=sim_id)
...
def plot_streamlines_with_lic(x, y, U, velocity_magnitude, experiment, Re, output_path, sim_id=None):
    """
