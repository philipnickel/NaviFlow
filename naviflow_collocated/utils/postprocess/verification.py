...
def poiseuille_verification(x, y, U, p, Re, output_path, sim_id=None):
    ...
    # u(y) = U_max * (1 - (y/h)^2), where U_max = 1.5 * U_avg
    r = np.linspace(-h, h, len(u_numerical))
    u_analytical = 1.5 * u_inlet * (1 - (r**2 / h**2))
    y_norm = np.linspace(0, 1, len(u_analytical))
    
    # Plot velocity profiles
    ax1.plot(u_numerical, y_norm, 'o-', color='tab:blue', label="Numerical", markersize=2, alpha=0.6)
    ax1.plot(u_analytical, y_norm, '--', color='tab:orange', label="Analytical", linewidth=2)
    
    ax1.set_title("Velocity Profile at x=L/2", fontsize=12, pad=10)
    ax1.set_xlabel("u-velocity", fontsize=10)
    ...
    # Extract pressure along channel centerline using a more robust method
    y_center = H/2
    
    # Find unique x-coordinates to sample along
    unique_x_coords = np.unique(x)
    x_centerline = []
    p_centerline = []
    
    for x_coord in unique_x_coords:
        # Find all points at this x-station
        x_mask = np.isclose(x, x_coord)
        y_at_x = y[x_mask]
        p_at_x = p[x_mask]
        
        if y_at_x.size > 0:
            # Find the point closest to the geometric centerline
            closest_idx = np.argmin(np.abs(y_at_x - y_center))
            x_centerline.append(x_coord)
            p_centerline.append(p_at_x[closest_idx])
    
    x_centerline = np.array(x_centerline)
    p_centerline = np.array(p_centerline)
    ...
    ax2.set_ylabel(r"$\Delta$p", fontsize=10)
    ax2.grid(True, alpha=0.3)
