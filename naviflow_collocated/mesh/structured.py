...
class StructuredMesh(Mesh):
    def __init__(
        ...
        if is_uniform:
            self.x_nodes = np.linspace(xmin, xmax, self.nx)
            self.y_nodes = np.linspace(ymin, ymax, self.ny)
        else:
            self.x_nodes = self._generate_clustered_axis(
                xmin, xmax, self.nx, clustering_factor, refine_x
            )
            self.y_nodes = self._generate_clustered_axis(
                ymin, ymax, self.ny, clustering_factor, refine_y
            )
    ...
    def _generate_clustered_axis(self, a, b, n, beta, side):
        s = np.linspace(-1, 1, n)
        if side == "left":
            s = -np.tanh(beta * (1 - s)) / np.tanh(beta)
        elif side == "right":
            s = np.tanh(beta * (1 + s)) / np.tanh(beta)
        elif side == "both":
            s = np.tanh(beta * s) / np.tanh(beta)
        return a + 0.5 * (b - a) * (1 + s)
    ...
    def _generate_faces_and_cells(self):
        ...
        for i in range(nx - 1):
            for j in range(ny - 1):
                ...
                vol = (self.x_nodes[i + 1] - self.x_nodes[i]) * (
                    self.y_nodes[j + 1] - self.y_nodes[j]
                )
    ...
    def _compute_geometry(self):
        ...
        for i, face in enumerate(self._faces):
            ...
            n = np.array([d[1], -d[0]])
            l2 = np.linalg.norm(n)
            original_normals[i] = n / l2 if l2 > 1e-12 else np.array([0.0, 0.0])
    ...
    def plot(
        ...
        if show_nodes:
            ...
            if node_ids:
                for i, (x, y) in enumerate(nodes):
                    ax.text(
                        x, y, f"{i}", color="red", fontsize=6, ha="right", va="bottom"
                    )

        if show_centers:
            cell_centers = self.get_cell_centers()
            ax.plot(
                cell_centers[:, 0], cell_centers[:, 1], "go", ms=3, label="Cell Centers"
            )
            if cell_ids:
        ...
        ax.set_title(title)
        ax.set_xlim(
            self.xmin - 0.1 * (self.xmax - self.xmin),
            self.xmax + 0.1 * (self.xmax - self.xmin),
        )
        ax.set_ylim(
            self.ymin - 0.1 * (self.ymax - self.ymin),
            self.ymax + 0.1 * (self.ymax - self.ymin),
        )
