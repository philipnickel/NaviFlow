...
def generate_custom_geometry(geom_type, **params):
    ...
    if geom_type == "naca":
        ...
        x_coords, y_coords = generate_naca_profile(
            naca_digits, chord_length, num_points
        )
        ...
        if not points or len(points) < 3:
            raise ValueError(
                "At least 3 points must be provided for 'points' geometry type"
            )
...
def generate(
    ...
    if obstacle:
        ...
        if obstacle_type == "circle":
            ...
            obstacle_boundary = gmsh.model.geo.addCurveLoop(
            ...
            try:
                ...
                print(
                    f"Created custom geometry with {len(points)} points and {len(lines)} lines"
                )
    ...
    if wake_refinement and obstacle_center and obstacle_width:
        ...
        wake_x_start = (
            obstacle_center[0] + obstacle_width / 2
        )  # Start at the right edge of obstacle
        ...
        h_wake = (
            h_min * 1.5
        )  # Wake refinement is slightly coarser than boundary refinement
        ...
        if use_math_eval:
            ...
            if expansion_factor > 1.0:
                ...
                gmsh.model.mesh.field.setNumber(
                    wake_field2, "Thickness", obstacle_width / 2
                )

        print(
            f"  Wake region: Start: x={wake_x_start:.3f}, y=[{wake_y_min_start:.3f}, {wake_y_max_start:.3f}]"
        )
        print(
            f"              End:   x={wake_x_end:.3f}, y=[{wake_y_min_end:.3f}, {wake_y_max_end:.3f}]"
        )
    ...
    if wake_field:
        ...
        if "wake_field2" in locals() and wake_field2 is not None:
            gmsh.model.mesh.field.setNumbers(
                field_min, "FieldsList", [field_threshold, wake_field, wake_field2]
            )
        else:
            gmsh.model.mesh.field.setNumbers(
                field_min, "FieldsList", [field_threshold, wake_field]
            )
    ...
    edges_np = np.array([node_map[int(n)] for n in edge_nodes], dtype=np.int64).reshape(
        -1, 2
    )
