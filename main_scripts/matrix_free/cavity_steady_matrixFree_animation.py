...
frames = []
save_interval = 20  # Save every 20 iterations for animation
...
def update_frame(frame_data):
    ax.clear()
    u_data = frame_data['u']
    u_display = u_data[1:imax, :].T
    
    contour = ax.contourf(X, Y, u_display, 50, cmap='jet')
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
    ax.set_title(f'Ux (Re = {Re:.0f}), Iteration: {frame_data["iteration"]}, Res: {frame_data["maxRes"]:.6f}')
    return contour
...
# Save animation
animation.save(f'cavity_flow_animation_Re{Re:.0f}.mp4', writer='ffmpeg', fps=10, dpi=150)
