...
def abbreviate_resolution(resolution):
    ...
    # Map specific mesh sizes to standard resolutions
    if resolution == '127x127':
        resolution = 'medium'
    elif resolution == '63x63':
        resolution = 'coarse'
...
def generate_caption(parsed_info):
    ...
    # Get the actual resolution for display (not the mapped one)
    display_resolution = parsed_info['resolution']
    if display_resolution == '127x127':
        display_resolution = '127x127 (medium)'
    elif display_resolution == '63x63':
        display_resolution = '63x63 (coarse)'
    
    caption = (f"{exp_name} simulation at Re={parsed_info['reynolds']} using "
               f"{parsed_info['mesh_type']} {display_resolution} mesh with "
               f"{parsed_info['scheme']} discretization scheme")
    
    # Add FMG information for power law cases based on robust config search
    if parsed_info['scheme'] == 'Power Law':
        found_fmg = False
        # Search all config and pseudo_config files under experiments/
        for fname in ['config.yaml', 'pseudo_config.yaml']:
            for path in glob.glob(os.path.join('experiments', '**', fname), recursive=True):
                if 'fmg' in path.lower():
                    try:
                        with open(path, 'r') as f:
                            config = yaml.safe_load(f)
                        # Check for match on experiment, mesh, resolution, Re
                        if (
                            str(config.get('experiment', '')).lower() == parsed_info['experiment'].lower() and
                            str(config.get('domain', {}).get('mesh', ['',''])[0]).lower() == parsed_info['mesh_type'].lower() and
                            str(config.get('domain', {}).get('mesh', ['',''])[1]).lower() == parsed_info['resolution'].lower() and
                            str(config.get('physical_properties', {}).get('reynolds_number', '')).lower() == str(parsed_info['reynolds']).lower()
                        ):
                            found_fmg = True
                            break
    ...
    caption += f" (Simulation ID: {parsed_info['sim_id'][:8]})"
