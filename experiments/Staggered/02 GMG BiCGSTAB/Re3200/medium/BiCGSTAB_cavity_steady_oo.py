...
Re=3200, 127x127 structured mesh with Power Law discretization
"""
...
nx, ny = 2**7-1, 2**7-1 # Grid size (127x127)
reynolds = 3200             # Reynolds number
alpha_p = 0.05             # Very conservative pressure relaxation
alpha_u = 0.2              # Very conservative velocity relaxation
max_iterations = 20000     # Maximum number of iterations
tolerance = 1e-9           # Extremely strict tolerance
h = 1/nx 
disc_order = 1
expected_disc_error = h**(disc_order)
pressure_tolerance = 1e-14  # Extremely strict pressure tolerance
...
momentum_solver = MatrixFreeMomentumSolver(tolerance=1e-14, max_iterations=30000, solver_type='bicgstab')
...
# Generate a unique simulation ID with random hash
...
metadata = {
    ...
    'Momentum solver tolerance': '1e-14',
