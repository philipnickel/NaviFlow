...
Re=400, 255x255 structured mesh with Power Law discretization
"""
...
nx, ny = 2**8-1, 2**8-1 # Grid size (255x255)
reynolds = 400             # Reynolds number
alpha_p = 0.2              # Lower pressure relaxation for stability  
alpha_u = 0.5              # Lower velocity relaxation for stability
max_iterations = 20000     # Maximum number of iterations
tolerance = 1e-8           # Very strict tolerance
h = 1/nx 
disc_order = 1
expected_disc_error = h**(disc_order)
pressure_tolerance = 1e-12  # Very strict pressure tolerance
...
pressure_solver = MatrixFreeBiCGSTABSolver(
    ...
    smoother_relaxation=1.0,
    smoother_method_type='red_black'
)
momentum_solver = MatrixFreeMomentumSolver(tolerance=1e-10, max_iterations=20000, solver_type='bicgstab')
...
metadata = {
    ...
    'Momentum solver tolerance': '1e-10',
    ...
    'Multigrid pre-smoothing steps': 3,
    'Multigrid post-smoothing steps': 3,
    'Multigrid smoother relaxation': 1.0,
